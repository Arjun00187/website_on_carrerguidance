...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nameErr) && empty($passErr) && empty($emailErr) && empty($birthErr) && empty($phnoErr) && empty($genErr) && empty($repassErr) && empty($fileuploadErr) && empty($addErr) && empty($fileuploadErr) && empty($cErr)) {
        ...
        $result=mysqli_query($conn,"insert into tw(name,email,pass,repass,exam,phno,gender,birthdate,address) values('$uname','$email','$pass','$repass','$exam','$phno','$gen','$birth','$add');") or $val='email already exists';
...
<body>
<a href="home.html" style="position: absolute; top: 50px; left: 50px;"><button style="background-color: greenyellow; color: black;padding: 10px 20px;border: none;border-radius: 5px;cursor: pointer;font-size: 16px;text-decoration: none;transition: background-color 0.3s ease;">HOME</button></a>
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" enctype="multipart/form-data">
        ...
        <option value="JEEADAVANCE">JEEADVANCE</option>
        <option value="APEMACET">APEAMCET</option>
        <option value="TSEMACET">TSEAMCET</option>
        ...
        Confirm Password: <input type="password" name="Repass" value="<?php echo $repass ?>">
        <span style="color:red"><?php echo $repassErr; ?></span><br><br>
        Mobile: <input type="text" name="phno" value="<?php echo $phno ?>">
        ...
        Dateofbirth: <input type="date" name="birth" value="<?php echo $birth ?>">
        ...
        Certificate: <input type="file" name="cupload">
